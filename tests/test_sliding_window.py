...
def _round(i, stim="stim", decision="dec", note=""):
    return ExplicitHistoryRound(heartbeat_id=i, stimulus_summary=stim,
...
def test_needs_compact_false_when_under_limit():
    w = SlidingWindow(history_token_budget=10000)
...
def test_total_tokens_approximation_scales_with_content():
    w = SlidingWindow(history_token_budget=4096)
    small = SlidingWindow(history_token_budget=4096)
