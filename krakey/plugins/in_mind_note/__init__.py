...
modifier (writer) and the tool (reader) import ``_CACHE_KEY``
...
# Slot name in PluginContext.plugin_cache where build_modifier stashes
# its InMindModifierImpl instance for build_tool (loaded next) to
# pick up. See plugins/dashboard/__init__.py for the same pattern with
# WebChatHistory.
_CACHE_KEY = "in_mind_modifier"
