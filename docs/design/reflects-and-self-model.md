...
具体 hook 点按 kind 定义。例如 `kind="hypothalamus"` 的 Reflect 覆盖"翻译 Self Decision → tool calls"这一步。
...
**Self 怎么更新它**：
- 通过新的 tool `update_in_mind`（由 Reflect 在 `attach(runtime)` 时
...
- `tool.py`: `UpdateInMindTool` (is_internal=True, 直接调 Reflect.update)
- `prompt.py`: `IN_MIND_INSTRUCTIONS_LAYER` 常量 + 虚拟 round 渲染
- `reflect.py`: `InMindReflectImpl` 主体 + `attach(runtime)` 注册 tool
