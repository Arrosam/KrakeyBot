...
combines three pieces, kept in their own files:
  ...
  * ``gm/query.py``    — ``GMQueryMixin``: vec_search / fts_search /
                          ...
                          delegating to ``src/memory/tools/`` (parameterized
...
Callers continue to use ``from src.memory.graph_memory import GraphMemory``
...
from src.memory._db import cosine_similarity  # noqa: F401  re-export for tests
from src.memory.gm.query import GMQueryMixin
from src.memory.gm.storage import (  # noqa: F401  re-exports for callers
    AsyncEmbedder,
    GMStorage,
    _row_to_node,
)
